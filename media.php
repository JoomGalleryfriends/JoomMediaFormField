 ...
 *
 * Modified by JoomGallery::ProjectTeam to support inserting images from JoomGallery.
 */
...
class JFormFieldMedia extends JFormField
{
	...
	protected function getInput()
	{
		...
		if (!self::$initialised)
		{
			...
			// Build the script.
			$script = array();
			$script[] = '	function jInsertFieldValue(value, id, type) {';
			...
			$script[] = '	function mediaField_selectimage(img_id, img_title, id, img_name, img_type) {';
			$script[] = '		var $ = jQuery.noConflict();';
      $script[] = '   var value = "index.php?option=com_joomgallery&view=image&format=raw&id=" + img_id + "&type=" + img_type';
			$script[] = '		var old_value = $("#" + id).val();';
			$script[] = '		if (old_value != value) {';
			$script[] = '			var $elem = $("#" + id);';
			$script[] = '			$elem.val(value);';
		...
		if ($this->disabled != true)
		{
			...
			$html[] = '<a class="modal btn" href="'
				. ($this->readonly ? ''
					: 'index.php?option=com_joomgallery&amp;view=mini&amp;format=raw&amp;catid=0&amp;extended=1&amp;object='.$this->id.'&amp;prefix=mediaField')
				. '" rel="{handler: \'iframe\', size: {x: 620, y: 550}}">';
			$html[] = '<span class="icon-picture"></span></a>';
      $html[] = '<a class="btn hasTooltip" title="'
